...
{?>
    ...
    <?php 
    $annoncesCorrespondantes = $pdo->query("SELECT * FROM annonce WHERE ville ="."'".$_GET['ville']."';");
    ...
    for($i=0; $i<count($tout); $i++)
    {
        ...
        else 
        {
            ...
            <a href="<?php echo 'annonce.php?id='.$tout[$i]["id_annonce"]?>"><?php echo $tout[$i]["titre"]?></a>
            <h5><?php echo $tout[$i]['ville']?></h5>
            <h4>RÉSERVÉ !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! fallait aller plus vite enculé<br><h4>
            </div>
            <?php
        }
